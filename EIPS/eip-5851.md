...
```solidity
    ...
    Defines the condition encoded for the identity index 1,that holder must be equal to or more than 18 years old.
    **/
    ...
    **/
    ...
    /// @notice function for assigning the SBT badge with given credentials to the given user address `claim` that fullfills the terms defined by the `StandardClaim` stored in `SBTID`.
    /// @dev it should only be called by the issuer address and MUST emit the Certified event as exaplined in next session. 
    ...
    */
    ...
    /// @notice function for revoking the SBT assigned to the given certificate (once its analysed that standardClaim is no further valid). 
    /// @dev it should only be called by the issuer address and MUST emit the Revoked event as explained below
    /// @param certifying is the address of the holder of the given SBTID that is to be revoked.
    /// @param SBTID is the identifier of the SBT i.e is to be revoked for the 
...
1. Implementation of write functions on SBT (i.e `changeStandardClaim()`, `certify()` and `revoke()`) are dependent on the admin role (which MUST be held by issuer) and thus issuer needs to manage the keys securely.
